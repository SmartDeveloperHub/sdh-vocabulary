...
final class ImmutableDocumentationProvider implements DocumentationProvider {
	...
	static ImmutableDocumentationProvider create(final Module module) {
		final Path root=module.context().root();
		final String path =
			DocumentationStrategy.
				rootPath(
					module.context().base(),
					module.implementationIRI());
		return new ImmutableDocumentationProvider(root.getParent().resolve(path));
	}
